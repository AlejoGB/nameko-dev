...
def get_orders(page: int=0, limit: int=50, rpc = Depends(get_rpc)):
    try:
        return _get_orders(rpc, page, limit)
    ...
    try:
        with nameko_rpc.next() as nameko:
            for item in order_data['order_details']:
                nameko.products.get(item['product_id'])
